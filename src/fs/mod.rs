...
//! - A [`Directory`] type, which leverages the file descriptor and open syscall's
...
//! - Distinct absolute ([`Path<Abs>`](Path<Abs>)) and relative
//!   ([`Path<Rel>`](Path<Rel>)) path types with additional formatting invariants for both.
